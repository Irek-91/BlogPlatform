...
export class TokensRepository {
    ...
    async addRefreshToken(newDeviceAndRefreshToken: devicesMongo): Promise<boolean | null> {
        try {
            const res = await DevicesModelClass.insertMany({ ...newDeviceAndRefreshToken })
            return true
        }
        catch (e) { return null }
    }
    ...
    async getUserIdByDeviceId(deviceId: string): Promise<ObjectId | null> {
        ...
        try {
            const res = await DevicesModelClass.findOne({ deviceId: deviceId }).lean();
    ...
    async deleteTokenAndDevice(issuedAt: string): Promise<true | null> {
        try {
            const res = await DevicesModelClass.deleteOne({ issuedAt: issuedAt })
            if (res === null) { return null }
            return true
        }
        catch (e) { return null }
    ...
    async getTokenAndDevice(userId: ObjectId): Promise<devicesMongo[] | null> {
        ...
        try {
            const res = await DevicesModelClass.find({ userId: userId }).lean();
            if (res === null) { return null }
            return res
        }
        catch (e) { return null }
    ...
    async deleteDeviceId(deviceId: string): Promise<null | boolean> {
        try {
            const res = await DevicesModelClass.deleteOne({ deviceId: deviceId });
            if (res === null) { return null }
    ...
    async findOneDeviceId(deviceId: string): Promise<devicesMongo | null> {
        try {
            const res = await DevicesModelClass.findOne({ deviceId: deviceId });
            if (res === null) { return null }
            return res
        }
        catch (e) { return null }
